...
function ChainBadge({ chain }: { chain: string }) {
    ...
    if (isSol) {
        color = '#0e7490'; // Darker teal for visibility on white
    ...
    return (
        <span
            className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border border-current opacity-90"
...
function UseCaseCard({ company, role, chains, description, quote, person, highlights, source, accent, logo }: UseCaseCardProps) {
    return (
        <div
            className="group rounded-3xl overflow-hidden border bg-white/60 backdrop-blur-xl transition-all duration-500 hover:bg-white/80 flex flex-col h-full p-6 md:p-8"
            style={{
