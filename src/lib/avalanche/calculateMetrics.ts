import { AvalancheNode, AvalancheOVHNode, AvalancheDashboardMetrics, ProviderBreakdownEntry } from '@/types';
import { AvalancheProviderCategorizationResult } from './filterOVH';
import { extractAvalancheIP } from './fetchPeers';
import { PROVIDER_COLORS, PROVIDER_LABELS } from '@/lib/config/constants';
import { getCountryFromMaxMind } from '@/lib/asn/maxmind';

const MARKET_SHARE_THRESHOLD = 5; // providers below this % go into "Others"

/**
 * Calculate all Avalanche dashboard metrics from peer data.
 *
 * Notable difference vs Solana: there is no `activatedStake` field.
 * Instead we surface `observedUptime` (0–100) per peer, and compute:
 *   - avgOVHUptime    : mean uptime across OVH nodes
 *   - pctOVHUptimeAbove80 : % of OVH nodes with uptime ≥ 80%
 *
 * These are displayed in the Analytics tab of the Avalanche dashboard.
 */
export function calculateAvalancheMetrics(
    ...
    // ── Uptime metrics (Avalanche-specific) ───────────────────────────────────
    let avgOVHUptime: number | undefined;
    let pctOVHUptimeAbove80: number | undefined;

    if (ovhNodeCount > 0) {
        const total = ovhNodes.reduce((acc, n) => acc + (n.observedUptime ?? 0), 0);
        avgOVHUptime = total / ovhNodeCount;
        const above80 = ovhNodes.filter(n => (n.observedUptime ?? 0) >= 80).length;
        pctOVHUptimeAbove80 = (above80 / ovhNodeCount) * 100;
    }

    // ── Top validators list ───────────────────────────────────────────────────
    const topValidators = ovhNodes.slice(0, 10);
