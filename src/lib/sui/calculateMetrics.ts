import { SuiValidator, SuiOVHNode, SuiDashboardMetrics } from '@/types/sui';
import { ProviderBreakdownEntry } from '@/types/dashboard';
import { SuiProviderCategorizationResult } from './filterOVH';
...
export function calculateSuiMetrics(
    allValidators: SuiValidator[],
    ovhNodes: SuiOVHNode[],
    providerCategorization: SuiProviderCategorizationResult,
): SuiDashboardMetrics {
    const totalNodes = allValidators.length;
    ...
    // ── Voting Power calculation (Sui-specific) ───────────────────────────────
    let ovhVotingPowerShare = 0;
    if (ovhNodes.length > 0) {
        const totalNetworkVotingPower = allValidators.reduce((acc, v) => acc + parseInt(v.votingPower ?? '0'), 0);
        const ovhTotalVotingPower = ovhNodes.reduce((acc, v) => acc + parseInt(v.votingPower ?? '0'), 0);
        
        if (totalNetworkVotingPower > 0) {
            ovhVotingPowerShare = (ovhTotalVotingPower / totalNetworkVotingPower) * 100;
        }
    }

    // ── Top validators list (already filtered by OVH) ──────────────────────────
    // Sort by voting power descending
    const topValidators = [...ovhNodes].sort((a, b) => parseInt(b.votingPower) - parseInt(a.votingPower));
